...
- For the current `transcribe-rs` ONNX adapters, `detected_language` may be `null`; `segments` is populated only when `timestamps=true` and the backend exposes real segment metadata.
